...
const ClubItem = ({ item }) => {
    const categories = item.categories || ["tag1", "tag2", "tag3"]; // Use categories from API if available
    const [logoUri, setLogoUri] = useState(null);
    ...
    return (
        <div className="club-item-card" style={{
            ...
            <div className="club-header" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                    <h3 style={{ fontSize: '1.5em', fontWeight: 'bold', margin: 0 }}>{item.clubName}</h3>
            ...
            {item.description && <p style={{ fontSize: '1em', color: '#333' }}>{item.description}</p>}
            {item.instagram && <a href={item.instagram} target="_blank" rel="noopener noreferrer" style={{ color: '#0066cc', textDecoration: 'underline' }}>{item.instagram}</a>}
